...
internal static class MediatorEmitter
{
    ...
    private static void EmitHeader(CodeWriter w)
    {
        ...
        w.Line("#pragma warning disable CS8600, CS8601, CS8602, CS8603, CS8604");
        w.Line();
        w.Line("using System;");
    ...
    private static void EmitSendWithResponse(CodeWriter w, Dictionary<string, HandlerInfo> requestHandlers)
    {
        ...
        w.Line("private static global::System.Threading.Tasks.Task<TResponse> SendUnknown<TResponse>(global::MDator.IRequest<TResponse> request, global::System.Threading.CancellationToken cancellationToken)");
        w.OpenBrace();
        w.Line("throw new global::System.InvalidOperationException($\"MDator: no handler known at compile time for request type '{request.GetType().FullName}'. Make sure the project containing the handler is built with the MDator source generator, or add [assembly: MDator.KnownRequest(typeof(...))] for dynamically-dispatched requests.\");");
        w.CloseBrace();
        w.Line();
    }
    ...
    private static void EmitSendVoid(CodeWriter w, Dictionary<string, HandlerInfo> voidHandlers)
    {
        ...
        w.Line("default:");
        w.Indent();
        w.Line("throw new global::System.InvalidOperationException($\"MDator: no void handler known for request type '{request!.GetType().FullName}'.\");");
        w.Dedent();
        w.CloseBrace();
        w.CloseBrace();
        w.Line();
        ...
        Dictionary<string, HandlerInfo> voidHandlers)
    {
        ...
        w.Line("default:");
        w.Indent();
        w.Line("throw new global::System.InvalidOperationException($\"MDator: no handler known for request type '{request.GetType().FullName}'.\");");
        w.Dedent();
        w.CloseBrace();
        w.CloseBrace();
        w.Line();
    ...
    private static void EmitCreateStream(CodeWriter w, Dictionary<string, HandlerInfo> streamHandlers)
    {
        ...
        w.Line("default:");
        w.Indent();
        w.Line("throw new global::System.InvalidOperationException($\"MDator: no stream handler known for request type '{request!.GetType().FullName}'.\");");
        w.Dedent();
        w.CloseBrace();
        w.CloseBrace();
        w.Line();
    ...
    {
        // When there are no stream handlers at all, emit a non-async method that
        // throws immediately — an async iterator without any yield statement is a
        // compile error, and we'd rather not emit dead methods.
